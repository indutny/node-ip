...
ip.toString = (buff, offset, length) => {
    offset = ~~offset;
...
ip.isV4Format = ip => ipv4Regex.test(ip);
ip.isV6Format = ip => ipv6Regex.test(ip);
...
ip.isPrivate = addr => /^(::f{4}:)?10\.([0-9]{1,3})\.([0-9]{1,3})\.([0-9]{1,3})$/i.test(addr) || /^(::f{4}:)?192\.168\.([0-9]{1,3})\.([0-9]{1,3})$/i.test(addr) || /^(::f{4}:)?172\.(1[6-9]|2\d|30|31)\.([0-9]{1,3})\.([0-9]{1,3})$/i.test(addr) || /^(::f{4}:)?127\.([0-9]{1,3})\.([0-9]{1,3})\.([0-9]{1,3})$/i.test(addr) || /^(::f{4}:)?169\.254\.([0-9]{1,3})\.([0-9]{1,3})$/i.test(addr) || /^f[cd][0-9a-f]{2}:/i.test(addr) || /^fe80:/i.test(addr) || /^::1$/.test(addr) || /^::$/.test(addr);
ip.isPublic = addr => !ip.isPrivate(addr);
ip.isLoopback = addr => /^(::f{4}:)?127\.([0-9]{1,3})\.([0-9]{1,3})\.([0-9]{1,3})/.test(addr) || /^fe80::1$/.test(addr) || /^::1$/.test(addr) || /^::$/.test(addr);
ip.loopback = family => { //
    // Default to `ipv4`
    //
    family = _normalizeFamily(family);
...
ip.fromLong = ipl => `${ipl>>>24}.${ipl>>16&255}.${ipl>>8&255}.${ipl&255}`;
