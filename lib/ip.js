...
ip.loopback = function loopback(family) {
  return ip.loopbackInterface(family).address;
};

ip.loopbackInterface = function loopbackInterface(family) {
  //
  // Default to `ipv4`
  //
  family = _normalizeFamily(family);
...
//
ip.networkInterface = function networkInterface(name, family) {
  ...
  if (name && name !== 'private' && name !== 'public') {
    return interfaces[name].find(function (details) {
      return details.family.toLowerCase() === family;
    });
  }

  //
  // Note: name will only be `public` or `private` or undefined
  // when this is called.
  //
  function matchNameAndFamily(details) {
    return details.family.toLowerCase() === family &&
      !ip.isLoopback(details.address) &&
      (!name || ((name === 'public') === ip.isPublic(details.address)));
  }

  for (var nic in interfaces) {
    if (interfaces.hasOwnProperty(nic)) {
      var d = interfaces[nic].find(matchNameAndFamily);
      if (d) {
        return d;
      }
    }
  ...
  return ip.loopbackInterface(family);
};

ip.address = function address(name, family) {
  var ni = ip.networkInterface(name, family);
  return ni ? ni.address : undefined;
};
